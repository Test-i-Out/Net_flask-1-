...
def generate_mock_devices():
    ...
    for i in range(1, 251):
        devices.append({
            ...
            "lastSeen": (datetime.now() - timedelta(days=random.randint(0, 7))).isoformat(),
...
def export_csv():
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=firemon-results-{device_id}-{datetime.now().strftime("%Y%m%d")}.csv'
